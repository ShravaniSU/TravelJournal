export const places = [
  {
    slug: 'golden-temple',
    name: 'Golden Temple, Amritsar',
    region: 'Amritsar, Punjab',
    coverImage:
      'https://upload.wikimedia.org/wikipedia/commons/9/94/The_Golden_Temple_of_Amrithsar_7.jpg',
    coverAlt: 'The Golden Temple reflected in the holy Amrit Sarovar at night',
    shortDescription:
      'The spiritual crown of Sikhism — where golden light reflects on holy water and 100,000 souls are fed daily in an act of divine equality.',
    intro:
      'Harmandir Sahib, popularly known as the Golden Temple, is a place of profound peace and selfless service. It is the most sacred shrine of Sikhism, welcoming people of all faiths with open arms.',
    story:
      'Entering the complex, the first sight of the golden sanctum rising from the blue waters of the sarovar is breathtaking. Walking the parikrama, the air is filled with the soothing sounds of Gurbani. The spirit of "Seva" is palpable everywhere, especially in the Langar hall where volunteers work tirelessly to feed thousands.',
    significance:
      'Founded by Guru Ram Das Ji and completed by Guru Arjan Dev Ji, the temple symbolizes human brotherhood and equality. The four entrances in four directions signify that people from all walks of life are welcome.',
    personalExperience:
      'Sitting by the sarovar at dawn, watching the first rays of sun hit the gold, I felt a deep sense of calm. The simplicity of the service and the power of the prayers make this place truly universal.',
    highlights: ['Harmandir Sahib', 'Amrit Sarovar', 'Langar (Community Kitchen)'],
    gallery: [
      {
        src: 'https://www.namasteindiatrip.com/blog/wp-content/uploads/2019/04/Golden-Temple-Punjab.jpg',
        alt: 'Golden Temple facade close up',
      },
      {
        src: 'https://globalgaz.com/wp-content/uploads/2019/09/DSC02190.jpg',
        alt: 'Langar preparation',
      },
      {
        src: 'https://assets.cntraveller.in/photos/60ba1b39d2d37295460ae7ec/master/w_1600%2Cc_limit/MHKYW5-866x577.jpg',
        alt: 'Night view of the Golden Temple',
      },
    ],
  },
  {
    slug: 'devprayag',
    name: 'Devprayag, Uttarakhand',
    region: 'Devprayag, Uttarakhand',
    coverImage:
      'https://uttarakhandtourism.gov.in/assets/media/UTDB_media_1736140340download_0.jpeg',
    coverAlt: 'Confluence of Alaknanda and Bhagirathi rivers at Devprayag',
    shortDescription:
      'The sacred meeting point where two mighty rivers become the Ganga — a serene blend of spirituality, mythology, and Himalayan beauty.',
    intro:
      'Devprayag is one of the Panch Prayag (five sacred river confluences) in Uttarakhand, where the Alaknanda and Bhagirathi rivers merge to form the holy Ganga. It holds immense religious significance in Hinduism and is a peaceful retreat nestled in the Himalayas.',
    story:
      'As you stand overlooking the confluence, the contrast of the two rivers is striking — one emerald green, the other a deeper shade. Watching them merge into a single powerful stream feels symbolic, almost meditative. The surrounding temples and mountain silence amplify the sacred energy of the place.',
    significance:
      'According to Hindu belief, this is the exact point where the Ganga officially begins its journey. Devprayag is also associated with sage Dev Sharma, after whom the town is named, and has been a site of pilgrimage for centuries.',
    personalExperience:
      'Sitting quietly above the sangam, listening to the sound of merging waters, felt grounding in a way that’s hard to explain. It’s not crowded or overwhelming — just raw, peaceful, and deeply spiritual.',
    highlights: ['Sangam', 'Raghunathji Temple', 'Dashrathshila Temple'],
    gallery: [
      {
        src: 'https://sannidhi.net/wp-content/uploads/2023/07/devprayag2.png',
        alt: 'Panoramic view of Devprayag town',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/AjitHota_BirthPlaceOfGanges.jpg/1280px-AjitHota_BirthPlaceOfGanges.jpg',
        alt: 'Close view of river confluence',
      },
      {
        src: 'https://t.eucdn.in/tourism/lg-jpg/devprayag-9286235.jpg',
        alt: 'Devprayag temples and ghats',
      },
    ],
  },
  {
    slug: 'dhara-devi',
    name: 'Dhara Devi Temple',
    region: 'Srinagar, Uttarakhand',
    coverImage:
      'https://t.eucdn.in/tourism/lg/dhari-devi-temple-895254.webp',
    coverAlt: 'The Dhari Devi Temple situated on the banks of the Alaknanda river',
    shortDescription:
      'The fierce and protective goddess who guards the gateway to the Himalayas, enshrined at the edge of the mighty Alaknanda river.',
    intro:
      'Dhari Devi Temple is dedicated to Goddess Dhari, the guardian deity of Uttarakhand. Located on the banks of the Alaknanda River, it is a site of immense spiritual power and natural beauty.',
    story:
      'The temple has a unique setting, perched above the flowing waters. Legend says the upper half of the goddess is here, while the lower half is in Kalimath. The atmosphere is charged with the sound of the river and the bells of the devotees.',
    significance:
      'She is considered the protector of the Char Dham. Her power is deeply revered by the locals and pilgrims alike.',
    personalExperience:
      'Standing by the river, watching the goddess’s face which is said to change throughout the day, was a mystical experience. The connection between the deity and the river is profound.',
    highlights: ['Guardian of Char Dham', 'Riverside Shrine', 'Shakti Peeth'],
    gallery: [
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/5/5c/Dhari_Devi.jpg',
        alt: 'Dhari Devi Temple and Alaknanda River',
      },
      {
        src: 'https://www.manchalamushafir.com/dhari-devi-temple/images/dhari-devi-temple-uttarakhand.webp',
        alt: 'Temple entrance and bridge',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Dhari_Devi_Temple_Jan_2002.jpg/500px-Dhari_Devi_Temple_Jan_2002.jpg',
        alt: 'Alaknanda river in the mountains',
      },
    ],
  },
  {
    slug: 'kedarnath',
    name: 'Kedarnath',
    region: 'Rudraprayag, Uttarakhand',
    coverImage:
      'https://triptradition.com/wp-content/uploads/2024/02/Kedarnath-yatra-package-from-Delhi.jpg',
    coverAlt:
      'Kedarnath temple framed by snow-covered Himalayan peaks under a moody sky',
    shortDescription:
      'A high-altitude pilgrimage where stone, snow, and silence make every step feel devotional and deeply humbling.',
    intro:
      'Kedarnath is one of those places where the landscape itself feels like prayer. The temple stands with striking resilience among towering Himalayan peaks, and the journey toward it becomes as meaningful as the destination.',
    story:
      'Reaching Kedarnath is an experience shaped by altitude, weather, and devotion. The path winds through dramatic mountain scenery, and with every turn the atmosphere grows quieter, more reverent, and more inward-looking. When the temple finally comes into view, the setting feels almost unreal, as if the mountains themselves are holding the shrine in stillness. The cold air, the ringing bells, and the slow rhythm of pilgrims create a powerful sense of arrival that lingers long after the visit is over.',
    significance:
      'Kedarnath is one of the twelve Jyotirlingas of Lord Shiva and a central site in the Char Dham Yatra. Its spiritual importance is amplified by its remoteness, symbolizing perseverance, surrender, and faith. The temple is also historically associated with Adi Shankaracharya, who is believed to have revived pilgrimage traditions in the region.',
    personalExperience:
      'What stayed with me most was the contrast between physical exhaustion and emotional calm. The trek demanded effort, but the closer I got to the temple, the lighter everything felt. Kedarnath left me with a rare kind of quiet that was both grounding and uplifting.',
    highlights: ['High-altitude shrine', 'Stone architecture', 'Jyotirlinga'],
    gallery: [
      {
        src: 'https://www.shivkhori.in/wp-content/uploads/2025/09/Kedarnath-Temple.webp',
        alt: 'Snow-capped Himalayan mountain peaks at sunrise',
      },
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_XgRDxJPx8jayonnv7zE5Km-uSbo0bamt7Q&s',
        alt: 'Alpine forest path through Himalayan mountains',
      },
      {
        src: 'https://media.istockphoto.com/id/515855602/photo/kedarnath-in-india.jpg?s=612x612&w=0&k=20&c=_KmnQbbvqRftKusDTU_JUjVM3V80Mo2jBqaCNx948us=',
        alt: 'Misty mountain valley in the Himalayas',
      },
    ],
  },
  {
    slug: 'triyugi-narayan',
    name: 'Triyugi Narayan',
    region: 'Rudraprayag, Uttarakhand',
    coverImage:
      'https://upload.wikimedia.org/wikipedia/commons/7/7c/Triyuginarayan_Temple.jpg',
    coverAlt: 'The ancient stone Triyuginarayan Temple, the legendary site of divine marriage',
    shortDescription:
      'A serene village temple believed to witness the eternal wedding of Shiva and Parvati, rich with sacred symbolism.',
    intro:
      'Triyugi Narayan carries a quieter kind of sacred power. The village setting feels intimate, but its mythology gives it immense depth and emotional resonance.',
    story:
      'Unlike the grand scale of the higher Himalayan shrines, Triyugi Narayan invites you into stillness through simplicity. The temple sits in a peaceful settlement, and its spiritual aura comes less from dramatic altitude and more from the stories held in the place. The atmosphere encourages slower observation: the temple courtyard, the traditional architecture, the mountain air, and the devotional memory attached to the eternal flame. It feels like a place where mythology remains close to daily life.',
    significance:
      'Triyugi Narayan is traditionally believed to be the site of the divine marriage of Lord Shiva and Goddess Parvati. The sacred fire, said to have burned since that wedding, is central to the temple’s identity and makes the place especially meaningful for couples and devotees seeking blessings for marriage and harmony.',
    personalExperience:
      'This was one of the most emotionally memorable stops for me because it felt deeply human as well as spiritual. There was gentleness in the environment, and the mythology made the entire visit feel tender, timeless, and reflective.',
    highlights: ['Wedding site', 'Eternal sacred flame', 'Quiet village atmosphere'],
    gallery: [
      {
        src: 'https://triyuginarayantemple.com/wp-content/uploads/2024/03/triyuginarayan-temple.jpg',
        alt: 'Serene mountain village in morning light',
      },
      {
        src: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEghVhoROTXcKKr6NhL1v93pDTa9-iavtcMX9VcWCdYxO5-u_zS3FbrhsjWq8EopvtzwKo_ZyoPw6I94Tj9lVwx1rJCaPiAZbhQvfra4BPzRjsNKp2AWnpxWv4LoC3dRKzuMu2_1Y2sjCtgQ/s1600/IMG_1845.JPG',
        alt: 'Ancient temple with mountain backdrop',
      },
      {
        src: 'https://blog.summitersadventures.com/uploads/images/2020/05/image_750x_5eca39d2771f1.jpg',
        alt: 'Akhand Dhuni',
      },
    ],
  },
  {
    slug: 'tungnath',
    name: 'Tungnath',
    region: 'Rudraprayag, Uttarakhand',
    coverImage:
      'https://www.trawell.in/admin/images/upload/387998757Kedarnath_Tunganath_Main.jpg',
    coverAlt:
      'Himalayan trekking trail and alpine meadow evocative of Tungnath',
    shortDescription:
      'A breathtaking temple trek through alpine meadows, where every rise opens into sky, stone, and sacred stillness.',
    intro:
      'Tungnath blends pilgrimage with one of the most beautiful short treks in the Himalayas. The journey upward is filled with open views, changing weather, and a sense of wonder that builds with each step.',
    story:
      'The route to Tungnath feels airy and expansive, with alpine meadows, layered ridgelines, and crisp mountain light stretching in every direction. The temple appears almost organically placed within the landscape, as though it belongs to the mountain itself. There is a beautiful simplicity to the experience: walking, breathing, watching the clouds shift, and arriving at a shrine that feels both ancient and intimate. The combination of movement and meditation makes Tungnath especially memorable.',
    significance:
      'Tungnath is one of the Panch Kedar temples dedicated to Lord Shiva and is often described as the highest Shiva temple in the world. Its setting reinforces the spiritual symbolism of ascent, offering a pilgrimage where nature and devotion are inseparable.',
    personalExperience:
      'Tungnath felt joyful. The trail was energizing, the scenery kept unfolding in dramatic ways, and the final approach to the temple felt deeply peaceful. It was one of those journeys where the walk itself became the prayer.',
    highlights: ['Panch Kedar temple', 'Scenic mountain trek', 'Jyotirlinga'],
    gallery: [
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQzA9nrQFWOJtFGcrTTeDClvoS1fGPjMwkqzw&s',
        alt: 'Alpine meadow path through high mountains',
      },
      {
        src: 'https://www.chardham-pilgrimage-tour.com/assets/images/tungnath-banner4.webp',
        alt: 'Snow-capped mountain peaks at high altitude',
      },
      {
        src: 'https://discoverwithdheeraj.com/wp-content/uploads/2014/10/106.jpg',
        alt: 'Mountain forest trail during Himalayan trek',
      },
    ],
  },
  {
    slug: 'badrinath',
    name: 'Badrinath',
    region: 'Chamoli, Uttarakhand',
    coverImage:
      'https://t.eucdn.in/tourism/lg/badrinath-9953735.webp',
    coverAlt: 'The vibrant and colorful Badrinath temple set against the majestic Himalayan peaks',
    shortDescription:
      'A vibrant Vaishnavite dham where sacred architecture, river energy, and mountain light come together beautifully.',
    intro:
      'Badrinath feels immediately alive with color and devotion. Set along the Alaknanda River and surrounded by dramatic peaks, it offers a more vibrant but equally contemplative spiritual atmosphere.',
    story:
      'The town of Badrinath carries a distinct rhythm. Pilgrims gather near the temple, the river flows briskly nearby, and the surrounding mountains create a powerful natural backdrop. The temple facade is bright and welcoming, standing out against the rugged landscape. Walking through the area, there is a shared sense of anticipation and reverence, but also warmth, conversation, and community. It is a place where spiritual practice feels woven into everyday movement.',
    significance:
      'Dedicated to Lord Vishnu, Badrinath is one of the Char Dham pilgrimage sites and among the most revered temples in India. It is deeply tied to Hindu sacred geography and is associated with Adi Shankaracharya, who helped re-establish it as an important center of worship.',
    personalExperience:
      'Badrinath felt expansive in a different way from Kedarnath. Instead of silence alone, I found energy here: ritual sounds, bright colors, and a sense that devotion could be both intimate and collective at the same time.',
    highlights: ['Char Dham', 'Riverside setting', 'Color-rich temple facade'],
    gallery: [
      {
        src: 'https://s7ap1.scene7.com/is/image/incredibleindia/2-mana-village-badrinath-uttarakhand-attr-hero?qlt=82&ts=1726646481060',
        alt: 'Sacred temple architecture in mountain setting',
      },
      {
        src: 'https://www.chardham-pilgrimage-tour.com/assets/images/chardham-banner7.webp',
        alt: 'River valley flowing through Himalayan mountains',
      },
      {
        src: 'https://pioneeredge.in/wp-content/uploads/2026/04/Badrinath-Dham-ritually-opened-to-devotees-all-Char-Dham-now-open-780x470.jpg',
        alt: 'Mountain landscape with forest and sky',
      },
    ],
  },
  {
    slug: 'mana-village',
    name: 'Mana Village, Uttarakhand',
    region: 'Near Badrinath, Uttarakhand',
    coverImage:
      'https://s7ap1.scene7.com/is/image/incredibleindia/2-bheem-pul-badrinath-uttarakhand-attr-hero?qlt=82&ts=1726646571390',
    coverAlt: 'Stone houses of Mana Village with Himalayan backdrop',
    shortDescription:
      'The last Indian village before the Tibet border — where mythology, mountains, and raw simplicity come together.',
    intro:
      'Mana Village, located just a few kilometers from Badrinath, is famously known as the last village of India before the Indo-Tibetan border. It is a serene Himalayan settlement rich in mythological and cultural significance.',
    story:
      'Walking through Mana feels like stepping into a different world — stone houses, narrow lanes, and locals wrapped in traditional woolens. Just beyond the village, you hear the roar of the Saraswati River emerging fiercely from the rocks. The air is crisp, and everything feels untouched and real.',
    significance:
      'According to Hindu mythology, this is the route the Pandavas took on their final journey to heaven (Swargarohini). The nearby Vyas Gufa is believed to be where sage Ved Vyasa composed the Mahabharata, with Lord Ganesha as the scribe.',
    personalExperience:
      'Standing near the “Last Indian Village” sign felt surreal — like you’re at the edge of the map. The quiet, the mountains, and the stories tied to this place make it feel deeply special.',
    highlights: ['Bhim Pul', 'Vyas Gufa', 'Saraswati River Origin'],
    gallery: [
      {
        src: 'https://static.toiimg.com/photo/99853435.cms',
        alt: 'Village pathway with stone houses',
      },
      {
        src: 'https://s7ap1.scene7.com/is/image/incredibleindia/1-mana-village-badrinath-uttarakhand-attr-hero?qlt=82&ts=1726646468125',
        alt: 'Bhim Pul rock bridge over Saraswati river',
      },
      {
        src: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/14/02/da/9f/img-20180626-104421-largejpg.jpg?w=900&h=500&s=1',
        alt: 'Saraswati River near Mana Village',
      },
    ],
  },
  {
    slug: 'narasimha-temple-joshimath',
    name: 'Narasimha Temple, Joshimath',
    region: 'Joshimath, Uttarakhand',
    coverImage:
      'https://hblimg.mmtcdn.com/content/hubble/img/joshimathdestimgs/mmt/activities/m_Narsingh_Temple_1_l_479_640.jpg',
    coverAlt: 'Ancient Narasimha Temple in Joshimath surrounded by hills',
    shortDescription:
      'A sacred winter abode of Badrinath — where devotion meets prophecy in the quiet Himalayan town of Joshimath.',
    intro:
      'The Narasimha Temple in Joshimath is dedicated to Lord Vishnu’s fierce incarnation, Narasimha. It holds immense importance as the winter seat of Lord Badrinath when the main shrine in Badrinath remains closed.',
    story:
      'Walking into the temple, the atmosphere feels ancient and deeply spiritual. The idol of Lord Narasimha is unique — with a left arm that is believed to be gradually thinning over time. Locals quietly speak of the prophecy tied to it, adding a mysterious aura to the place.',
    significance:
      'According to belief, when the arm of the Narasimha idol eventually falls, the route to Badrinath will be permanently blocked, marking the end of the current pilgrimage path. The temple is also closely linked to Adi Shankaracharya, who established Joshimath as a spiritual center.',
    personalExperience:
      'There’s a calm intensity here — not crowded, not loud, just deeply rooted faith. Sitting inside the temple, you feel a mix of peace and curiosity, especially knowing the legend tied to the idol.',
    highlights: ['Narasimha Idol', 'Winter Seat', 'Joshimath Monastic Heritage'],
    gallery: [
      {
        src: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEgZPHl48KnJ8Bvnwwvbu3igaVc15o0oVXr5McOmfucFfCQ0mOxkfkeL0kY9Vt6p1aqsEp5jqYD3baf-EiMUqxt1FkJLJNCKXIULQX5PVLFrNCmvu8OtbwbUmk3G-RMI22-chLLP1u3Bp4M/s1600/Joshimath+Narsing_Courtesy-NallaniKrishna.JPG',
        alt: 'Temple exterior view',
      },
      {
        src: 'https://sannidhi.net/wp-content/uploads/2023/07/narsingh2.png',
        alt: 'Temple courtyard',
      },
      {
        src: 'https://thetempleguru.com/wp-content/uploads/2024/08/Narsingh-Temple-Joshimath-1.jpg',
        alt: 'Joshimath town and surroundings',
      },
    ],
  },
  {
    slug: 'haridwar',
    name: 'Haridwar, Uttarakhand',
    region: 'Haridwar, Uttarakhand',
    coverImage:
      'https://images.travelandleisureasia.com/wp-content/uploads/sites/2/2025/04/22095901/EvergreenFeatured-2025-04-22T095848.327-1600x900.jpg',
    coverAlt: 'Evening Ganga Aarti at Har Ki Pauri with lamps floating on the river',
    shortDescription:
      'Where the Ganga enters the plains — a city of rituals, devotion, and glowing aartis that light up the soul.',
    intro:
      'Haridwar is one of the seven holiest cities in India, where the sacred Ganga River descends from the Himalayas to the plains. It is a major pilgrimage destination, known for its ghats, temples, and timeless spiritual atmosphere.',
    story:
      'As evening falls, Har Ki Pauri transforms into something magical. Priests perform the Ganga Aarti with synchronized chants, fire lamps, and ringing bells, while thousands gather along the ghats. The sight of diyas floating on the river creates a surreal, almost hypnotic scene.',
    significance:
      'Haridwar is an important site for Hindu rituals, including the Kumbh Mela, one of the largest religious gatherings in the world. Taking a dip in the Ganga here is believed to cleanse sins and bring spiritual liberation.',
    personalExperience:
      'Standing by the river during aarti, surrounded by chants and flickering flames, felt incredibly powerful. It’s chaotic yet calming at the same time — a place where faith feels alive in every moment.',
    highlights: ['Har Ki Pauri', 'Ganga Aarti', 'Mansa Devi Temple'],
    gallery: [
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/thumb/1/11/Haridwar_Ganga_6.JPG/1280px-Haridwar_Ganga_6.JPG',
        alt: 'Crowd during Ganga Aarti',
      },
      {
        src: 'https://static.toiimg.com/photo/48099242.cms',
        alt: 'Har Ki Pauri daytime view',
      },
      {
        src: 'https://images.trvl-media.com/place/6126242/9dda7923-985e-461c-893a-fb50ea75eaca.jpg',
        alt: 'Mansa Devi Temple view',
      },
    ],
  },
  {
    slug: 'khatu-shyam-temple',
    name: 'Khatu Shyam Temple, Rajasthan',
    region: 'Khatu, Sikar, Rajasthan',
    coverImage:
      'https://substack-video.s3.amazonaws.com/video_upload/post/177431678/dc334af2-c667-4ccd-babb-30cfb2846bfa/transcoded-1761705286.png',
    coverAlt: 'Khatu Shyam Temple decorated with flowers and lights',
    shortDescription:
      'A sacred temple of unwavering faith — where devotees believe the warrior Barbarika blesses with instant grace.',
    intro:
      'Khatu Shyam Temple is one of the most revered pilgrimage sites in Rajasthan, dedicated to Shyam Baba, believed to be the incarnation of Barbarika from the Mahabharata. The temple attracts millions of devotees throughout the year.',
    story:
      'As you enter the temple town, chants of “Shyam Baba ki Jai” echo all around. The atmosphere is filled with devotion, music, and color. Inside, the beautifully adorned idol of Shyam Baba, covered in flowers and jewels, creates a powerful emotional connection for devotees.',
    significance:
      'According to legend, Barbarika was granted a boon by Lord Krishna to be worshipped in Kaliyuga as Shyam. Devotees believe that praying here with a true heart fulfills wishes and brings relief from hardships.',
    personalExperience:
      'The energy here is intense and uplifting — even in the crowd, there’s a strong sense of faith and surrender. It feels less like a visit and more like an emotional experience.',
    highlights: ['Shyam Baba Idol', 'Falgun Mela', 'Sacred Kund (Shyam Kund)'],
    gallery: [
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9d/Khatu_Shyam_Ji_-_panoramio.jpg/1280px-Khatu_Shyam_Ji_-_panoramio.jpg',
        alt: 'Temple entrance view',
      },
      {
        src: 'https://i.ytimg.com/vi/VpUjeZRHzjM/maxresdefault.jpg',
        alt: 'Idol of Shyam Baba',
      },
      {
        src: 'https://thecsrjournal.in/wp-content/uploads/2025/09/9916_khatu-shyam-mandir-rajasthan.webp',
        alt: 'Crowd during Falgun Mela',
      },
    ],
  },
  {
    slug: 'mahakal',
    name: 'Mahakal, Ujjain',
    region: 'Ujjain, Madhya Pradesh',
    coverImage:
      'https://cdn.mahakal.brainabove.net/public-static/bg_4.jpg',
    coverAlt: 'The majestic Mahakaleshwar Temple in Ujjain at twilight',
    shortDescription:
      'The fierce and majestic Jyotirlinga where time itself is worshipped as the Lord of Lords.',
    intro:
      'Mahakaleshwar is more than a temple; it is the center of the universe for many. Here, Lord Shiva is worshipped as Mahakal, the Lord of Time and Death.',
    story:
      'Entering the Mahakal complex in Ujjain feels like stepping into a different dimension. The air is thick with the scent of incense and the rhythmic chanting of "Jai Mahakal". The most profound experience is the Bhasma Aarti, a pre-dawn ritual where the deity is bathed in sacred ash.',
    significance:
      'Mahakaleshwar is one of the twelve Jyotirlingas and is unique because it is dakshinamukhi (south-facing). It is also one of the 18 Shakti Peethas.',
    personalExperience:
      'Watching the evening aarti by the Kshipra river after visiting the temple was a moment of pure serenity. Mahakal taught me about the cycle of life.',
    highlights: ['Bhasma Aarti', 'Kshipra River', 'Southfacing Jyotirlinga'],
    gallery: [
      {
        src: 'https://m.media-amazon.com/images/I/61Fx8McRlML._AC_UF894,1000_QL80_.jpg',
        alt: 'Temple corridor with ancient carvings',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/thumb/7/75/Mahakaleshwar_Temple%2C_Ujjain.jpg/1280px-Mahakaleshwar_Temple%2C_Ujjain.jpg',
        alt: 'Evening aarti lamps reflecting in water',
      },
      {
        src: 'https://cdn.mahakal.brainabove.net/public-static/BhasmaAarti.jpg',
        alt: 'Ancient stone shikhara of Mahakal temple',
      },
    ],
  },
  {
    slug: 'harsiddhi-mata-temple',
    name: 'Harsiddhi Mata Temple, Ujjain',
    region: 'Ujjain, Madhya Pradesh',
    coverImage:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTjfeaZGhnO0Gg61DgTbUw4w-0tFc7-xVNsrQ&s',
    coverAlt: 'Harsiddhi Mata Temple with its iconic दीप स्तंभ (lamp towers)',
    shortDescription:
      'A powerful Shakti Peetha where devotion glows through towering lamps and ancient faith fills the air.',
    intro:
      'Harsiddhi Mata Temple in Ujjain is one of the prominent Shakti Peethas in India, dedicated to Goddess Harsiddhi, an incarnation of Durga. Located near the famous Mahakaleshwar Temple, it holds deep spiritual significance among devotees.',
    story:
      'As you enter the temple complex, the first thing that grabs attention is the pair of towering दीप स्तंभ (lamp pillars). During festivals, especially Navratri, these are lit with hundreds of lamps, creating a breathtaking golden glow. The temple courtyard feels vibrant yet deeply rooted in tradition.',
    significance:
      'According to legend, this is the place where a part of Goddess Sati’s body fell, making it a sacred Shakti Peetha. The temple was also a center of worship during the reign of King Vikramaditya, who was a devoted follower of the goddess.',
    personalExperience:
      'Seeing the lamp towers lit up at dusk was something else — calm, powerful, and almost hypnotic. The energy here feels intense but grounding at the same time.',
    highlights: ['Deep Stambh (Lamp Towers)', 'Shakti Peeth', 'Navratri Celebrations'],
    gallery: [
      {
        src: 'https://bhasmarti.com/wp-content/uploads/2025/01/image-4-1024x576.png',
        alt: 'Temple front view',
      },
      {
        src: 'https://images.news18.com/ibnkhabar/uploads/2025/04/harsiddhi-mandir-2025-04-0f043137cf59d46aa05e623b7db8a383.jpg?impolicy=website&width=640&height=480',
        alt: 'Illuminated lamp towers',
      },
      {
        src: 'https://www.trawell.in/admin/images/upload/891041148Harsiddhi_Mata_Temple.jpg',
        alt: 'Temple courtyard during festival',
      },
    ],
  },
  {
    slug: 'kal-bhairav-temple-ujjain',
    name: 'Kal Bhairav Temple, Ujjain',
    region: 'Ujjain, Madhya Pradesh',
    coverImage:
      'https://www.trawell.in/admin/images/upload/891041742Sri_Kalbhairav_Temple.jpg',
    coverAlt: 'Kal Bhairav Temple entrance with devotees offering prayers',
    shortDescription:
      'A mysterious temple where offerings of liquor are accepted by the deity — blending devotion with ancient tantric traditions.',
    intro:
      'Kal Bhairav Temple in Ujjain is dedicated to Lord Kal Bhairav, a fierce manifestation of Lord Shiva. It is one of the most unique and powerful temples in India, deeply rooted in tantric and spiritual practices.',
    story:
      'As you approach the temple, the energy feels different — intense and raw. Devotees line up holding bottles of liquor as offerings, which priests pour into a shallow plate before the deity. What’s fascinating is that the liquid is believed to mysteriously disappear, adding to the temple’s aura of mystery.',
    significance:
      'Kal Bhairav is considered the guardian deity (Kotwal) of Ujjain. The temple is associated with tantric rituals and is mentioned in ancient texts. Offering liquor here is an age-old tradition believed to please the deity and remove obstacles.',
    personalExperience:
      'The vibe here is unlike typical temples — powerful, slightly mysterious, and deeply rooted in belief. It makes you question and admire the diversity of spiritual practices in India.',
    highlights: ['Liquor Offering', 'Kal Bhairav Idol', 'Tantric Significance'],
    gallery: [
      {
        src: 'https://cdn.shopify.com/s/files/1/1328/9161/files/kal_bhairav_temple_ujjain_480x480.jpg?v=1661066144',
        alt: 'Temple exterior view',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/6/6b/Kal_Bhairav_temple_Ujjain.jpg',
        alt: 'Idol of Kal Bhairav',
      },
      {
        src: 'https://www.bhasmarti.com/wp-content/uploads/2025/01/kalabhairav-Internet-2a841e.jpg',
        alt: 'Devotees offering prayers',
      },
    ],
  },
  {
    slug: 'omkareshwar',
    name: 'Omkareshwar',
    region: 'Khandwa, Madhya Pradesh',
    coverImage:
      'https://touringwithpk.com/wp-content/uploads/2025/01/IMG_2545-2a.jpg',
    coverAlt: 'The sacred island of Omkareshwar on the Narmada River',
    shortDescription:
      'A sacred island shaped like the holy symbol "Om", cradled by the gentle flow of the Narmada river.',
    intro:
      'Omkareshwar is a place of profound peace. The entire island is shaped like the sacred symbol "Om", making it one of the most spiritually significant sites in India.',
    story:
      'Crossing the bridge to the Omkareshwar temple, you feel the cool breeze from the Narmada river. The temple is built into the hillside, and its narrow passages lead you to the ancient Jyotirlinga. The rhythm of the river and the bells creates a meditative atmosphere.',
    significance:
      'One of the 12 Jyotirlingas, Omkareshwar is unique for its island setting. It is associated with King Mandhata and has been a center of pilgrimage for thousands of years.',
    personalExperience:
      'Taking a boat ride around the island at sunset was magical. The reflection of the temple in the water and the distant sounds of chanting made it unforgettable.',
    highlights: ['Om-shaped island', 'Narmada river boat ride', 'Jyotirlinga'],
    gallery: [
      {
        src: 'https://chardham.ticketstotrip.com/wp-content/uploads/2025/08/omkareshwar-temple-island.webp',
        alt: 'Narmada river flowing past the temple',
      },
      {
        src: 'https://dharohar.hargharpuja.com/wp-content/uploads/2025/04/Omkareshwar-jyotirlinga.jpg',
        alt: 'Temple entrance at sunrise',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/0/01/Omkareswar_Jyotirlinga.jpg',
        alt: 'Devotees crossing the bridge',
      },
    ],
  },
  {
    slug: 'kal-bhairav-temple-varanasi',
    name: 'Kal Bhairav Temple, Varanasi',
    region: 'Varanasi, Uttar Pradesh',
    coverImage:
      'https://feeds.abplive.com/onecms/images/uploaded-images/2023/08/27/034ac99c87c95d349c15cb4cc830c8b91693151206128367_original.jpg?impolicy=abp_cdn&imwidth=600',
    coverAlt: 'Kal Bhairav Temple entrance in Varanasi',
    shortDescription:
      'The fierce guardian of Kashi — where Lord Bhairav watches over the city and grants protection to devotees.',
    intro:
      'Kal Bhairav Temple in Varanasi is dedicated to Lord Kal Bhairav, considered the Kotwal (protector and ruler) of Kashi. No pilgrimage to Varanasi is considered complete without seeking his blessings.',
    story:
      'Tucked within the bustling streets of Varanasi, the temple has a strong, commanding presence. Devotees come with deep respect, as Bhairav is not just worshipped but revered as the ultimate authority of the city. The idol, adorned with garlands and a silver face mask, feels intense yet protective.',
    significance:
      'According to belief, Lord Kal Bhairav governs Kashi and keeps a record of the deeds of those who visit. It is said that even Lord Shiva granted him authority over the city, making him an essential part of the spiritual journey in Varanasi.',
    personalExperience:
      'There’s a certain seriousness in the air here — not intimidating, but powerful. It feels like a place where you become more aware of your actions and intentions.',
    highlights: ['Kotwal of Kashi', 'Silver-faced Idol', 'Essential Kashi Darshan Stop'],
    gallery: [
      {
        src: 'https://i.pinimg.com/736x/d7/8e/5c/d78e5c57f5904e1e4cd0f25587fafbaf.jpg',
        alt: 'Temple entrance crowded with devotees',
      },
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_dv95sIYGYfdaBpiQAOXdZHjhxSOKws92zQ&s',
        alt: 'Kal Bhairav idol with silver mask',
      },
      {
        src: 'https://cf-img-a-in.tosshub.com/lingo/gnt/images/story/202405/66431e05e9bd1-baba-kaal-bhairav-141708952-16x9.jpg',
        alt: 'Streets around the temple',
      },
    ],
  },
  {
    slug: 'kashi-vishwanath',
    name: 'Kashi Vishwanath',
    region: 'Varanasi, Uttar Pradesh',
    coverImage:
      'https://www.pilgrimagetour.in/blog/wp-content/uploads/2023/10/How-to-Reach-Kashi-Vishwanath-Temple.jpg',
    coverAlt: 'The golden dome of Kashi Vishwanath temple against the blue sky',
    shortDescription:
      'The Golden Temple of Kashi, where the eternal light of Shiva illuminates the world\'s oldest living city.',
    intro:
      'Kashi Vishwanath is the heart of Varanasi. It is a place where life and death meet, and where the spiritual energy of thousands of years is palpable in every stone.',
    story:
      'Walking through the narrow "galis" of Kashi, you finally arrive at the magnificent temple complex. The newly built corridor has opened up the view to the Ganga, making the transition from the river to the shrine seamless and breathtaking.',
    significance:
      'Varanasi is the holiest of the seven sacred cities (Sapta Puri). The temple is a Jyotirlinga and has been destroyed and rebuilt several times, symbolizing the eternal nature of faith.',
    personalExperience:
      'The morning Ganga Aarti followed by a visit to Vishwanath Ji was the highlight of my trip. The city is chaotic yet perfectly still at its core.',
    highlights: ['Jyotirlinga', 'Vishwanath Corridor', 'Ganga Aarti'],
    gallery: [
      {
        src: 'https://d18guwlcxyb2ak.cloudfront.net/wp-content/uploads/2019/03/13043614/CIty-Varanasi-Kashi-Vishwanath.jpg',
        alt: 'Ghats of Varanasi at dawn',
      },
      {
        src: 'https://content.skyscnr.com/m/26eaa06a2be696f0/original/GettyImages-525109131.jpg',
        alt: 'Corridor architecture',
      },
      {
        src: 'https://kashiyatra.in/wp-content/uploads/2023/10/dashashwamedh-ghat-evening-ganga-aarti.jpg',
        alt: 'Evening aarti on the Ganga',
      },
    ],
  },
  {
    slug: 'annapurna-temple-varanasi',
    name: 'Annapurna Temple, Varanasi',
    region: 'Varanasi, Uttar Pradesh',
    coverImage:
      'https://ayodhyavaranasitourism.com/wp-content/uploads/elementor/thumbs/Annapurna-Devi-Mandir-Varanasi-timings-qzicrsowy3xvfrux7wpf4uclli8drvjmsaivw8as20.jpg',
    coverAlt: 'Annapurna Devi Temple near Kashi Vishwanath in Varanasi',
    shortDescription:
      'The goddess of nourishment in the heart of Kashi — where food is sacred and no devotee leaves empty-handed.',
    intro:
      'Annapurna Temple in Varanasi is dedicated to Goddess Annapurna, the deity of food and nourishment. Located near the Kashi Vishwanath Temple, it holds deep spiritual importance for devotees seeking blessings of prosperity and sustenance.',
    story:
      'Walking through the narrow गलियाँ of Varanasi, the temple appears almost quietly, yet holds immense presence. Inside, the idol of Goddess Annapurna is adorned beautifully, symbolizing abundance. The idea that the goddess herself feeds the world adds a comforting, maternal energy to the place.',
    significance:
      'According to legend, Goddess Annapurna once fed Lord Shiva here, symbolizing that even the divine depends on nourishment. The temple represents the belief that food is sacred and should always be shared, making it an important cultural and spiritual symbol.',
    personalExperience:
      'There’s something deeply grounding about this temple — simple, warm, and welcoming. It reminds you of the importance of gratitude for even the smallest things, like a meal.',
    highlights: ['Goddess Annapurna', 'Anna Daan', 'Proximity to Kashi Vishwanath'],
    gallery: [
      {
        src: 'https://cdn.triptotemples.com/ImageManager/shri-annapurna-temple-the-temple-of-goddess-of-food-681c31ac7db8c142cd1b50d5-42442430.webp',
        alt: 'Temple entrance view',
      },
      {
        src: 'https://akm-img-a-in.tosshub.com/aajtak/images/story/202111/kashi_vishwanath_mandir_7375videocapture_20211106-1805471691382835739463590-sixteen_nine.jpg?size=1200:675',
        alt: 'Idol of Goddess Annapurna',
      },
      {
        src: 'https://dorituals.com/wp-content/uploads/2025/03/images-19-900x500.jpeg',
        alt: 'Nearby Varanasi ghats',
      },
    ],
  },
  {
    slug: 'vindyachala',
    name: 'Vindyachala Temple',
    region: 'Mirzapur, Uttar Pradesh',
    coverImage:
      'https://assets.zyrosite.com/cdn-cgi/image/format=auto,w=1920,fit=crop/m5K20JWKZ1fOByOP/vindhyachal-temple1-YX4XMr4qqgcgEzNW.jpeg',
    coverAlt: 'The historic Vindhyavasini Temple in the Vindhya range',
    shortDescription:
      'The abode of Goddess Vindhyavasini, one of the most powerful Shakti Peethas where divine feminine energy has been worshipped for millennia.',
    intro:
      'Vindhyachal is a major Shakti Peeth located in Mirzapur. It is the residence of Goddess Vindhyavasini, who is believed to be the protector and mother of the universe.',
    story:
      'The town of Vindhyachal is steeped in antiquity. Navigating the crowded lanes to reach the temple, one feels the ancient pulse of devotion. The temple itself is small but radiates a powerful energy that draws millions of pilgrims.',
    significance:
      'It is one of the few places where the goddess is worshipped in her complete form. It is also mentioned in the Durga Saptashati as the place where the goddess will reside to protect the world.',
    personalExperience:
      'The "Trikona Parikrama" (visiting three temples in a triangle) was a soul-stirring journey. It felt like a deep dive into the roots of Shakti worship in India.',
    highlights: ['Vindhyavasini Devi', 'Shakti Peeth', 'Trikona Parikrama'],
    gallery: [
      {
        src: 'https://vindhyachalmata.com/wp-content/uploads/2018/12/vindhyavasini-Aarti-and-Shringar-Timings-1.jpg',
        alt: 'Ancient temple architecture',
      },
      {
        src: 'https://content.jdmagicbox.com/comp/mirzapur/r8/9999p5442.5442.220317220426.g4r8/catalogue/vindhyachal-temple-vindhyachal-mirzapur-temples-aoNsaAk2BH.jpg',
        alt: 'Devotional offerings',
      },
      {
        src: 'https://akm-img-a-in.tosshub.com/aajtak/images/story/202310/vindhyachal-sixteen_nine.jpg',
        alt: 'Ghats near Vindhyachal',
      },
    ],
  },
  {
    slug: 'ayodhya',
    name: 'Ayodhya',
    region: 'Ayodhya, Uttar Pradesh',
    coverImage:
      'https://upload.wikimedia.org/wikipedia/commons/thumb/d/de/Shri_Ram_Janambhoomi_Mandir%2C_Ayodhya_Dham.jpg/1280px-Shri_Ram_Janambhoomi_Mandir%2C_Ayodhya_Dham.jpg',
    coverAlt: 'The magnificent Ram Mandir in Ayodhya under a golden sunset',
    shortDescription:
      'The grand homecoming of Lord Ram in a magnificent temple that blends ancient tradition with modern architectural glory.',
    intro:
      'Ayodhya is a city of celebration. The new Ram Mandir is not just a building; it is a symbol of a nation\'s soul returning to its roots.',
    story:
      'The atmosphere in Ayodhya is electric. The wide roads leading to the temple are lined with flags and murals. Entering the temple, the intricate carvings in pink sandstone take your breath away. The idol of Ram Lalla is mesmerizing.',
    significance:
      'The birthplace of Lord Ram, Ayodhya is one of the most important pilgrimage sites in Hinduism. The new temple is built in the Nagara style and is a masterpiece of Indian craftsmanship.',
    personalExperience:
      'I felt a sense of collective joy in Ayodhya. Everyone was smiling, chanting, and sharing the same feeling of devotion. It was truly a historic moment.',
    highlights: ['Ram Janmabhoomi', 'Nagara Architecture', 'Sarayu River'],
    gallery: [
      {
        src: 'https://c9admin.cottage9.com/uploads/5652/ayodhya-ram-mandir-murti.jpg',
        alt: 'Intricate carvings on temple pillars',
      },
      {
        src: 'https://www.tourmyindia.com/socialimg/ayodhya-tourism.jpg',
        alt: 'Temple flags fluttering in the wind',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/b/ba/Sarayu_River_night_view%2C_Ayodhya_001.jpg?utm_source=en.wikivoyage.org&utm_campaign=index&utm_content=original',
        alt: 'Evening lights in Ayodhya',
      },
    ],
  },
  {
    slug: 'mahakumbh-2025',
    name: 'Mahakumbh 2025',
    region: 'Prayagraj, Uttar Pradesh',
    coverImage:
      'https://images.mid-day.com/images/images/2025/feb/2mahakumbh_d.jpg',
    coverAlt: 'A vast gathering of devotees at the Sangam during Kumbh Mela',
    shortDescription:
      'The world\'s largest spiritual congregation, where millions unite at the sacred Sangam for a bath of immortality.',
    intro:
      'Mahakumbh 2025 is set to be the grandest spiritual event of our time. It is a celebration of faith that transcends boundaries and time.',
    story:
      'Imagine a city of tents that pops up overnight. The energy of millions of people walking towards the Sangam—the confluence of Ganga, Yamuna, and Saraswati—is indescribable. The sound of conch shells and the smell of woodfire fill the air.',
    significance:
      'The Mahakumbh happens every 12 years at Prayagraj. It is believed that taking a dip in the Sangam during this time cleanses one of all sins and leads to liberation.',
    personalExperience:
      'I am preparing for this journey with great anticipation. The idea of being part of such a massive human gathering focused entirely on spirituality is humbling.',
    highlights: ['Triveni Sangam dip', 'Naga Sadhu procession', 'City of Tents'],
    gallery: [
      {
        src: 'https://imgs.etvbharat.com/etvbharat/prod-images/15-01-2025/1200-675-23329238-1083-23329238-1736939574788.jpg',
        alt: 'Devotees at the Sangam',
      },
      {
        src: 'https://www.livemint.com/lm-img/img/2025/01/13/600x338/An-aerial-view-of-Sangam-during-Maha-Kumbh-Mela-at_1736741306630_1736741314391.jpg',
        alt: 'Sadhu at the Kumbh Mela',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Pantoon_bridge_view_01.jpg/1280px-Pantoon_bridge_view_01.jpg',
        alt: 'Aerial view of the temporary city',
      },
    ],
  },
  {
    slug: 'pashupatinath',
    name: 'Pashupatinath',
    region: 'Kathmandu, Nepal',
    coverImage:
      'https://upload.wikimedia.org/wikipedia/commons/1/1f/Pashupatinath_Temple-2020.jpg',
    coverAlt: 'The golden roof of Pashupatinath temple by the Bagmati river',
    shortDescription:
      'The holiest Hindu temple in Nepal, where the Bagmati river witnesses the cycle of life and the eternal presence of Shiva.',
    intro:
      'Pashupatinath is the spiritual heart of Kathmandu. Dedicated to Lord Shiva as the Lord of Animals, it is a UNESCO World Heritage site known for its profound energy.',
    story:
      'Walking along the banks of the Bagmati river, you feel the weight of history and devotion. The most striking experience is the evening Bagmati Aarti, where flickering lamps and rhythmic chants create a hypnotic atmosphere.',
    significance:
      'Pashupatinath is one of the most important Shiva temples in the world. Its unique pagoda architecture and the sacredness of the Bagmati river make it a central pilgrimage site.',
    personalExperience:
      'Sitting on the opposite bank as the sun set was deeply reflective. The temple reflecting in the water brought a sense of peace that I have rarely found elsewhere.',
    highlights: ['Bagmati River Aarti', 'UNESCO World Heritage', 'Jyotirlinga'],
    gallery: [
      {
        src: 'https://prashantb.wordpress.com/wp-content/uploads/2023/02/pashupatinath.jpg',
        alt: 'Ancient shrines at Pashupatinath',
      },
      {
        src: 'https://www.travelhimalayan.com/wp-content/uploads/2026/01/Pashupatinath-to-Mount-Kailash-1.webp',
        alt: 'Sadhu at the temple complex',
      },
      {
        src: 'https://gstreksnepal.com/wp-content/uploads/2024/12/pashupati-banner-2-6393058c.webp',
        alt: 'Evening aarti preparations',
      },
    ],
  },
  {
    slug: 'manakamana',
    name: 'Manakamana',
    region: 'Gorkha, Nepal',
    coverImage:
      'https://admin.visitmanakamana.com/assets/images/pages/1646373481.jpg',
    coverAlt: 'The Manakamana temple ridge overlooking the Himalayas',
    shortDescription:
      'The "Wish Fulfilling" goddess perched high on a ridge, reachable by a scenic cable car journey through the clouds.',
    intro:
      'Manakamana Temple is dedicated to the Bhagwati goddess. Believed to grant the wishes of those who visit with a pure heart, it is one of Nepal\'s most popular pilgrimage sites.',
    story:
      'The journey is as memorable as the destination. The cable car ride takes you over lush green valleys and terraced fields, offering breathtaking views of the Trishuli river and the Himalayas.',
    significance:
      'Named from "Mana" (heart) and "Kamana" (wish), the temple dates back to the 17th century and is a significant site for both religious and cultural reasons.',
    personalExperience:
      'The sight of the temple surrounded by clouds was stunning. It felt like being on top of the world, both physically and spiritually.',
    highlights: ['Scenic CableCar', 'Wish-fulfilling', 'Himalayan Ridge Views'],
    gallery: [
      {
        src: 'https://cms.holidaystonepal.in/media/Blogs/Manakamana-Temple/Manakamana-Temple.png',
        alt: 'Cable car over the Trishuli river',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/5/52/Manakamana_Cable_Car_tower.jpg',
        alt: 'Temple ridge at sunrise',
      },
      {
        src: 'https://media.istockphoto.com/id/1478302060/photo/manakamana-mai-temple-nepali-architecture-tradition-in-kalupande-hills-indrasthan-kathmandu.jpg?s=612x612&w=0&k=20&c=hcPmbOpVPp2CdHX3TRfwiJyprKK9xyYKlMSPDKzRXuQ=',
        alt: 'Vibrant offerings at the temple',
      },
    ],
  },
  {
  slug: 'jagannath-temple-puri',
  name: 'Jagannath Temple',
  region: 'Puri, Odisha, India',
  coverImage:
    'https://upload.wikimedia.org/wikipedia/commons/b/b7/Shri_Jagannath_temple.jpg',
  coverAlt: 'The majestic Jagannath Temple tower rising above Puri',
  shortDescription:
    'One of the holiest Char Dham pilgrimage sites, where Lord Jagannath’s divine presence draws millions of devotees every year.',
  intro:
    'Jagannath Temple in Puri is among the most sacred Vaishnavite temples in India. Dedicated to Lord Jagannath, Balabhadra, and Subhadra, it is a spiritual heart of Odisha.',
  story:
    'The sound of conch shells, temple bells, and devotional chants fills the air around the massive temple complex. During Rath Yatra, the streets of Puri transform into a sea of devotion and celebration.',
  significance:
    'Built in the 12th century by King Anantavarman Chodaganga Deva, the temple is one of the Char Dham pilgrimage destinations and a major center of Hindu spirituality.',
  personalExperience:
    'Watching the temple flag flutter high above the sanctum while hearing distant kirtans created a feeling of timeless devotion and energy.',
  highlights: ['Char Dham', 'Rath Yatra Festival', 'Mahaprasad Tradition'],
  gallery: [
    {
      src: 'https://tirupatitirumalainfo.com/wp-content/uploads/2017/07/balabhadra_subhadra_jagannath.jpg',
      alt: 'Main entrance of Jagannath Temple',
    },
    {
      src: 'https://stampedmoments.com/wp-content/uploads/2025/06/Rath-Yatra-puri-jagannath-2.jpg',
      alt: 'Rath Yatra celebrations in Puri',
    },
    {
      src: 'https://i0.wp.com/iimun.in/blog/wp-content/uploads/2025/06/main-slider-2_1670308972.jpg?fit=924%2C601&ssl=1',
      alt: 'Temple complex from above',
    },
  ],
  },
  {
    slug: 'vimala-devi-shaktipeeth',
    name: 'Vimala Devi Shaktipeeth',
    region: 'Puri, Odisha, India',
    coverImage:
      'https://temple.yatradham.org/public/Product/temple/temple_Oibqzpb6_202408221428030.jpg',
    coverAlt: 'Ancient Vimala Devi shrine inside Jagannath Temple complex',
    shortDescription:
      'A revered Shaktipeeth where Goddess Vimala represents the divine feminine energy within the sacred Jagannath Temple complex.',
    intro:
      'Vimala Devi Temple is one of the important Shaktipeeths in India and is located inside the Jagannath Temple complex in Puri.',
    story:
      'Though smaller than the main Jagannath shrine, the temple carries immense spiritual depth. Devotees quietly gather here seeking blessings from the fierce yet compassionate goddess.',
    significance:
      'It is believed that the feet of Goddess Sati fell here, making it one of the sacred Shaktipeeths. Mahaprasad from Jagannath Temple becomes complete only after being offered to Vimala Devi.',
    personalExperience:
      'The calmness inside the shrine felt deeply grounding, especially amid the vibrant energy of the larger temple complex.',
    highlights: ['Shakti peeth', 'Ancient Goddess', 'Inside Jagannath Complex'],
    gallery: [
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS9yMcL1kMOI8kk0wiuZ9s4TRhAUAkRXTxOmQ&s',
        alt: 'Entrance of Vimala Devi Temple',
      },
      {
        src: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEg0ExC-qbQo6iKi6ZcLJulYZDgCOCvMFqso8-2IHJF2eDuJkDItIVFeK6toNh1EMebPvy-7noNbn3u2A90fDaD6eFhT6izqJrAqBupDXrYJ3mnIFE3sEwbya7t4XkYdcM-TC9342yfa3f_N/s1600/bahula+devi.jpg',
        alt: 'Stone carvings around the shrine',
      },
      {
        src: 'https://api.blessingsonthenet.com/uploads/microsite/46c2ccbdc51e51c269d220c422597689-1699023840667-mahalaxmi-temple-kolhapur_3.jpg',
        alt: 'Devotees offering prayers at Vimala temple',
      },
    ],
  },
  {
    slug: 'mahalakshmi-temple-puri',
    name: 'Mahalakshmi Temple',
    region: 'Puri, Odisha, India',
    coverImage:
      'https://bhubaneswartourism.in/images/tourist-places/lakshmi-temple-puri/lakshmi-temple-2.jpg',
    coverAlt: 'Sacred Mahalakshmi shrine inside Puri temple complex',
    shortDescription:
      'A divine shrine dedicated to Goddess Mahalakshmi, revered as the guardian of prosperity and the kitchen of Lord Jagannath.',
    intro:
      'The Mahalakshmi Temple in Puri holds a special place in Jagannath tradition, where the goddess is worshipped as the divine provider of nourishment and abundance.',
    story:
      'Pilgrims often visit this shrine before partaking in Mahaprasad, believing the blessings of Mahalakshmi sanctify the sacred food offered in the temple.',
    significance:
      'The temple is deeply connected to the rituals and kitchen traditions of Jagannath Temple, one of the largest temple kitchens in the world.',
    personalExperience:
      'The peaceful atmosphere near the shrine offered a softer and quieter spiritual experience compared to the bustling main temple.',
    highlights: ['Temple Kitchen', 'Prosperity Goddess', 'Jagannath Rituals'],
    gallery: [
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS9i9ERJuNaVf66L6RsnLFoHcQT8Pbwl84GpQ&s',
        alt: 'Entrance to Mahalakshmi shrine',
      },
      {
        src: 'https://assets.odishabytes.com/wp-content/uploads/2021/02/prasad-e1612967514206.jpg',
        alt: 'Temple rituals in progress',
      },
      {
        src: 'https://www.orissapost.com/wp-content/uploads/2020/06/This-legend-highlights-casteless-culture-of-Mahalakshmi.jpg',
        alt: 'Devotees at the Mahalakshmi temple',
      },
    ],
  },
  {
    slug: 'tota-gopinath',
    name: 'Tota Gopinath',
    region: 'Puri, Odisha, India',
    coverImage:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR7l5kzYY6w3OEfpQbnrqr0_u1Xskadp59-TA&s',
    coverAlt: 'Deity of Tota Gopinath seated inside the temple sanctum',
    shortDescription:
      'A serene Krishna temple closely associated with Chaitanya Mahaprabhu and deep Gaudiya Vaishnava devotion.',
    intro:
      'Tota Gopinath Temple is a spiritually significant site in Puri, known for its beautiful deity of Lord Krishna in a rare seated posture.',
    story:
      'Hidden within peaceful lanes away from crowded pilgrimage routes, the temple feels intimate and deeply meditative. Devotional singing often fills the courtyard.',
    significance:
      'The temple is closely linked to Chaitanya Mahaprabhu, who spent his later years in Puri immersed in Krishna bhakti.',
    personalExperience:
      'The quiet devotional atmosphere and soft chanting inside the temple created a feeling of closeness and emotional warmth.',
    highlights: ['Gaudiya Vaishnavism', 'Seated Deity', 'Chaitanya Mahaprabhu'],
    gallery: [
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSI6w1X0XfmYoW2BJjSXXDrAYSXQei5ijSvyQ&s',
        alt: 'Temple courtyard at Tota Gopinath',
      },
      {
        src: 'https://www.tirthayatra.org/wp-content/uploads/2023/09/Untitled-design-5-1.jpg',
        alt: 'Devotees singing bhajans',
      },
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcThjjIeuDGGtWYVQjn-mOJjtJzhg8tQUDWB_Q&s',
        alt: 'Sacred shrine interior',
      },
    ],
  },
  {
    slug: 'konark-sun-temple',
    name: 'Konark Sun Temple',
    region: 'Konark, Odisha, India',
    coverImage:
      'https://suryainn.in/wp-content/uploads/2023/09/konark-time-table.jpg',
    coverAlt: 'The iconic stone chariot structure of Konark Sun Temple',
    shortDescription:
      'A magnificent UNESCO World Heritage monument shaped like a celestial stone chariot dedicated to the Sun God.',
    intro:
      'Konark Sun Temple is one of India’s greatest architectural masterpieces, famous for its intricate carvings and grand chariot-like design.',
    story:
      'As sunlight falls across the ancient stone wheels and sculptures, the temple appears alive with stories carved centuries ago.',
    significance:
      'Built in the 13th century by King Narasimhadeva I, the temple symbolizes the Sun God Surya riding across the heavens.',
    personalExperience:
      'Walking around the massive carved wheels and weathered sculptures felt like stepping into a timeless work of art.',
    highlights: ['UNESCO World Heritage', 'Stone Chariot', 'Ancient Sculptures'],
    gallery: [
      {
        src: 'https://kevinstandagephotography.wordpress.com/wp-content/uploads/2020/03/ksp_6402-2-1.jpg?w=1024',
        alt: 'Front view of Konark temple',
      },
      {
        src: 'https://upload.wikimedia.org/wikipedia/commons/3/34/Sun_Temple_Konark_Puri_District_Odisha.jpg?utm_source=commons.wikimedia.org&utm_campaign=index&utm_content=original',
        alt: 'Famous stone wheel carvings',
      },
      {
        src: 'https://cf-images.assettype.com/TNIE/import/2020/1/25/original/Konark_Sun_temple.jpg?w=1200&h=900&auto=format%2Ccompress&fit=max&enlarge=true',
        alt: 'Temple sculptures at sunset',
      },
    ],
  },
  {
    slug: 'alarnath',
    name: 'Alarnath',
    region: 'Brahmagiri, Odisha, India',
    coverImage:
      'https://www.go2india.in/orissa/images/alarnath-temple.jpg',
    coverAlt: 'Ancient Alarnath temple in Brahmagiri',
    shortDescription:
      'A sacred Vishnu temple where devotees gather during the period when Lord Jagannath is believed to rest away from public view.',
    intro:
      'Alarnath Temple is an important pilgrimage destination associated with Lord Vishnu and deeply connected to Jagannath traditions.',
    story:
      'During Anavasara, when Jagannath Temple remains closed, thousands of devotees travel here seeking darshan and spiritual comfort.',
    significance:
      'The temple is especially important in Gaudiya Vaishnavism and is linked with the devotional journeys of Chaitanya Mahaprabhu.',
    personalExperience:
      'The atmosphere felt simple yet spiritually powerful, with devotees singing bhajans in deep devotion.',
    highlights: ['Anavasara Pilgrimage', 'Vishnu Worship', 'Gaudiya Vaishnava'],
    gallery: [
      {
        src: 'https://universeheaven.org/wp-content/uploads/2025/06/1000532994.jpg',
        alt: 'Temple courtyard at Alarnath',
      },
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRldJw7ySzX0FTdQBf7xHax2vWKVcrXRegDBQ&s',
        alt: 'Devotees at Alarnath temple',
      },
      {
        src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQq16ndfGXOTYdUj5bXWlQ9nPBz33f1U8u-yg&s',
        alt: 'Ancient temple architecture',
      },
    ],
  },
  {
    slug: 'sakshi-gopal',
    name: 'Sakshi Gopal',
    region: 'Puri, Odisha, India',
    coverImage:
      'https://res.cloudinary.com/kmadmin/image/upload/v1724159266/kiomoi/sakshi_gopal_9452.jpg',
    coverAlt: 'Sacred Sakshi Gopal temple entrance',
    shortDescription:
      'A beloved Krishna temple tied to the legendary story of the deity acting as a divine witness for truth and devotion.',
    intro:
      'Sakshi Gopal Temple is dedicated to Lord Krishna and is one of the most spiritually cherished pilgrimage sites near Puri.',
    story:
      'The temple’s legend tells of Lord Krishna walking as a witness for a devoted disciple, symbolizing divine justice and faithfulness.',
    significance:
      'The name “Sakshi” means witness, and the temple stands as a reminder of truth, devotion, and the living presence of Krishna.',
    personalExperience:
      'The temple carried a warm village-like spiritual charm that felt deeply personal and welcoming.',
    highlights: ['Krishna Devotion', 'Divine Witness', 'Historic Pilgrimage Site'],
    gallery: [
      {
        src: 'https://shono.sangbadpratidin.in/wp-content/uploads/2022/10/Sakhi-gopal-1200x900.jpg',
        alt: 'Temple sanctum at Sakshi Gopal',
      },
      {
        src: 'https://www.iskconbangalore.org/blog//wp-content/uploads/2015/07/Sakshi-Gopal.jpg',
        alt: 'Devotees offering prayers',
      },
      {
        src: 'https://www.astroved.com/astropedia/assets/images/temples/Sakshi-Gopal-Temple-Puri.jpg',
        alt: 'Temple architecture details',
      },
    ],
  },
  {
    slug: 'lingraj-temple',
    name: 'Lingraj Temple',
    region: 'Bhubaneswar, Odisha, India',
    coverImage:
      'https://static.toiimg.com/photo/47158773/.jpg',
    coverAlt: 'Towering Lingraj Temple in Bhubaneswar',
    shortDescription:
      'A majestic masterpiece of Kalinga architecture dedicated to Lord Shiva, standing as the spiritual soul of Bhubaneswar.',
    intro:
      'Lingraj Temple is one of the oldest and grandest temples in Odisha, dedicated to Harihara, a combined form of Shiva and Vishnu.',
    story:
      'The towering spire dominates the Bhubaneswar skyline, while the scent of incense and continuous chants create a deeply sacred atmosphere.',
    significance:
      'Built in the 11th century, the temple represents the pinnacle of Kalinga temple architecture and remains an active center of Shaivite worship.',
    personalExperience:
      'Seeing the temple rise above the city at sunrise felt awe-inspiring, especially with the sound of bells echoing through the complex.',
    highlights: ['Kalinga Architecture', 'Ancient Temple', 'Harihara Worship'],
    gallery: [
      {
        src: 'https://s7ap1.scene7.com/is/image/incredibleindia/lingaraj-temple-bhubaneshwar-odisha-2-attr-hero?qlt=82&ts=1742165154990',
        alt: 'Temple complex from above',
      },
      {
        src: 'https://s7ap1.scene7.com/is/image/incredibleindia/lingaraj-temple-bhubaneshwar-odisha-3-attr-about?qlt=82&ts=1742182882691',
        alt: 'Detailed carvings on the temple tower',
      },
      {
        src: 'https://img-cdn.publive.online/fit-in/640x430/filters:format(webp)/odishatv/media/post_attachments/uploadimage/library/16_9/16_9_0/Shivratri_1620278957.jpg',
        alt: 'Evening prayers at Lingraj Temple',
      },
    ],
  },
];
